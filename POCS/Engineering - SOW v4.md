...
| ERP System             | SAP ECC6 (Migration in progress)                  |
...
- Whether partial GRN is allowed or strictly full GRN
	- internal comment (makrand): partial grn and balance will be treated as shrinkage/credit note.
...
- Whether owned store secondary billing follows exact franchise rules or separate policy
...
#### FLOW 1: Primary Billing – Franchise → Baramati
...
4. System validates:
   ...
   - Price from SAP
5. Franchise completes payment via gateway
...
- ❌ Manual pricing not allowed
- ❌ Delivery without invoice not allowed
...
##### Constraints
- ❌ Billing without customer phone not allowed
- ❌ Sale without payment not allowed
...
- Whether customer invoices sync back to SAP
...
4. Sales Order manually created in SAP
5. Delivery & invoice manually executed
6. Secondary invoice issued to customer

##### Constraints
- ❌ Automated SO creation not allowed
- ✔ Manual SAP intervention required
...
#### Constraints
- ❌ Backend cannot trigger delivery → invoice conversion
- ✔ Billing executed manually in SAP
...
3. Payment must be completed via payment gateway
